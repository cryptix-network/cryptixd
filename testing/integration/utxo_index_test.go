...
func TestUTXOIndex(t *testing.T) {
	// Setup a single cryptixd instance
	...
	cryptixd, teardown := setupHarness(t, harnessParams)
	defer teardown()
	...
	getCoinSupplyResponse, err := cryptixd.rpcClient.GetCoinSupply()
	if err != nil {
	...
	for i := 0; i < transactionAmountToSpend; i++ {
		...
		_, err = cryptixd.rpcClient.SubmitTransaction(rpcTransaction, transactionID, false)
		if err != nil {
	...
	utxosByAddressesResponse, err := cryptixd.rpcClient.GetUTXOsByAddresses([]string{miningAddress1})
	if err != nil {
...
func buildTransactionForUTXOIndexTest(t *testing.T, entry *appmessage.UTXOsByAddressesEntry) (*appmessage.RPCTransaction, string) {
	...
	payeeAddress, err := util.DecodeAddress(miningAddress1, util.Bech32PrefixCryptixSim)
	if err != nil {
