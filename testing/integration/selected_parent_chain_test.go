...
func TestVirtualSelectedParentChain(t *testing.T) {
	// Setup a couple of cryptixd instances
	cryptixd1, cryptixd2, _, teardown := standardSetup(t)
	defer teardown()
	...
	err := cryptixd1.rpcClient.RegisterForVirtualSelectedParentChainChangedNotifications(true,
	...
	// In cryptixd2, mine a different chain of `blockAmountToMine` + 1
	// blocks over the genesis
	var chain2Tip *externalapi.DomainBlock
	for i := 0; i < blockAmountToMine+1; i++ {
		chain2Tip = mineNextBlock(t, cryptixd2)
	}

	// Connect the two cryptixds. This should trigger sync
	// between the two nodes
	connect(t, cryptixd1, cryptixd2)
	...
	// the first chain
	virtualSelectedParentChainFromChain1Tip, err := cryptixd1.rpcClient.GetVirtualSelectedParentChainFromBlock(
