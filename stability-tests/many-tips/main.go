...
func startNode() (teardown func(), err error) {
	...
	dataDir, err := common.TempDir("cryptixd-datadir")
	if err != nil {
		panic(errors.Wrapf(err, "Error in Tempdir"))
	}
	log.Infof("cryptixd datadir: %s", dataDir)

	cryptixdCmd, err := common.StartCmd("CRYPTIXD",
		"cryptixd",
	...
	spawn("startNode-cryptixdCmd.Wait", func() {
		err := cryptixdCmd.Wait()
		if err != nil {
			if atomic.LoadUint64(&shutdown) == 0 {
				panics.Exit(log, fmt.Sprintf("cryptixdCmd closed unexpectedly: %s. See logs at: %s", err, dataDir))
			}
			if !strings.Contains(err.Error(), "signal: killed") {
				// TODO: Panic here and check why sometimes cryptixd closes ungracefully
				log.Errorf("cryptixdCmd closed with an error: %s. See logs at: %s", err, dataDir)
			}
		}
	...
	return func() {
		...
		killWithSigterm(cryptixdCmd, "cryptixdCmd")
...
func mineLoopUntilHavingOnlyOneTipInDAG(rpcClient *rpc.Client, miningAddress util.Address) error {
	...
	cryptixMinerCmd, err := common.StartCmd("MINER",
		"cryptixminer",
		common.NetworkCliArgumentFromNetParams(activeConfig().NetParams()),
		"-s", rpcAddress,
	...
	spawn("cryptix-miner-Cmd.Wait", func() {
		err := cryptixMinerCmd.Wait()
		if err != nil {
			if atomic.LoadUint64(&shutdown) == 0 {
	...
	atomic.StoreUint64(&shutdown, 1)
	killWithSigterm(cryptixMinerCmd, "cryptixMinerCmd")
	return nil
}
