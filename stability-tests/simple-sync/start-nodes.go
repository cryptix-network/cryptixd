...
func startNodes() (teardown func(), err error) {
	...
	syncerDataDir, err := common.TempDir("cryptixd-datadir-syncer")
	if err != nil {
		panic(errors.Wrapf(err, "error in Tempdir"))
	}
	log.Infof("SYNCER datadir: %s", syncerDataDir)

	syncedDataDir, err := common.TempDir("cryptixd-datadir-synced")
	if err != nil {
		panic(errors.Wrapf(err, "error in Tempdir"))
	}
	log.Infof("SYNCED datadir: %s", syncedDataDir)

	syncerCmd, err := common.StartCmd("CRYPTIXD-SYNCER",
		"cryptixd",
	...
	syncedCmd, err := common.StartCmd("CRYPTIXD-SYNCED",
		"cryptixd",
		common.NetworkCliArgumentFromNetParams(activeConfig().NetParams()),
