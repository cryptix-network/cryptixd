...
func commandLoop(argsChan <-chan []string) ([]commandFailure, error) {
	...
	dataDirectoryPath, err := common.TempDir("cryptixdsanity-cryptixd-datadir")
	if err != nil {
	...
	for args := range argsChan {
		...
		cmd := exec.Command("cryptixd", args...)
		cmd.Stdout = common.NewLogWriter(log, logger.LevelTrace, "CRYPTIXD-STDOUT")
		cmd.Stderr = common.NewLogWriter(log, logger.LevelWarn, "CRYPTIXD-STDERR")
