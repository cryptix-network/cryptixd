...
func (csm *consensusStateManager) applyMergeSetBlocks(stagingArea *model.StagingArea, blockHash *externalapi.DomainHash,
	...
	for i, mergeSetBlock := range mergeSetBlocks {
		...
		if mergeSetBlock.Header.TimeInMilliseconds() > 0 {
			sourceBlockTime = uint64(mergeSetBlock.Header.TimeInMilliseconds())
		}
		atomicCreationContext := atomicstate.NewCreationContext(
			mergeSetBlockHash,
			mergeSetBlock.Header.DAAScore(),
			sourceBlockTime,
		)
...
func (csm *consensusStateManager) maybeAcceptTransaction(stagingArea *model.StagingArea,
	...
	blockDAAScore uint64, atomicCreationContext atomicstate.CreationContext,
	atomicGrowth *atomicstate.BlockStateGrowth) (isAccepted bool, accumulatedMassAfter uint64, err error) {
