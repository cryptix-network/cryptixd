/*
Package appmessage implements the cryptix appmessage protocol.

At a high level, this package provides support for marshalling and unmarshalling
supported cryptix messages to and from the appmessage. This package does not deal
with the specifics of message handling such as what to do when a message is
received. This provides the caller with a high level of flexibility.

# Cryptix Message Overview

The cryptix protocol consists of exchanging messages between peers. Each
message is preceded by a header which identifies information about it such as
which cryptix network it is a part of, its type, how big it is, and a checksum
...
of the currently supported cryptix messages are provided. For these supported
messages, all of the details of marshalling and unmarshalling to and from the
appmessage using cryptix encoding are handled so the caller doesn't have to concern
themselves with the specifics.

# Message Interaction

The following provides a quick summary of how the cryptix messages are intended
...
and write cryptix messages. The following sections provide a quick overview of
...
The cryptix network is a magic number which is used to identify the start of a
message and which cryptix network the message applies to. This package provides
...
As discussed in the cryptix message overview section, this package reads
and writes cryptix messages using a generic interface named Message. In
...
In order to unmarshall cryptix messages from the appmessage, use the ReadMessage
function. It accepts any io.Reader, but typically this will be a net.Conn to
a remote node running a cryptix peer. Example syntax is:

	// Reads and validates the next cryptix message from conn using the
	// protocol version pver and the cryptix network cryptixNet. The returns
	// are a appmessage.Message, a []byte which contains the unmarshalled
	// raw payload, and a possible error.
	msg, rawPayload, err := appmessage.ReadMessage(conn, pver, cryptixNet)
	if err != nil {
...
In order to marshall cryptix messages to the appmessage, use the WriteMessage
function. It accepts any io.Writer, but typically this will be a net.Conn to
a remote node running a cryptix peer. Example syntax to request addresses
from a remote peer is:
	...
	// pver, and the cryptix network cryptixNet. The return is a possible
	// error.
	err := appmessage.WriteMessage(conn, msg, pver, cryptixNet)
	if err != nil {
		// Log and handle the error
	}
