...
// Outpoint defines a cryptix data type that is used to track previous
// transaction outputs.
...
// NewOutpoint returns a new cryptix transaction outpoint point with the
...
// TxIn defines a cryptix transaction input.
type TxIn struct {
...
// NewTxIn returns a new cryptix transaction input with the provided
...
// TxOut defines a cryptix transaction output.
type TxOut struct {
	Value        uint64
	ScriptPubKey *externalapi.ScriptPublicKey
}

// NewTxOut returns a new cryptix transaction output with the provided
...
// MsgTx implements the Message interface and represents a cryptix tx message.
