...
func HandleGetConnectedPeerInfo(context *rpccontext.Context, _ *router.Router, _ appmessage.Message) (appmessage.Message, error) {
	...
	for _, peer := range peers {
		...
		info := &appmessage.GetConnectedPeerInfoMessage{
			...
			AdvertisedServices:        uint64(peer.Services()),
			IsHFAFastchain:            peer.Services()&appmessage.SFNodeHFAFastchain != 0,
			IsCryptixAtomic:           peer.Services()&appmessage.SFNodeCryptixAtomic != 0,
			IsStrongNodeClaims:        peer.Services()&appmessage.SFNodeStrongNodeClaims != 0,
			IsArchival:                peer.Services()&appmessage.SFNodeArchival != 0,
