...
import (
	...
	"github.com/cryptix-network/cryptixd/app/rpc"
...
// ComponentManager is a wrapper for all the cryptixd services
...
// Start launches all the cryptixd services.
...
// Stop gracefully shuts down all the cryptixd services.
func (a *ComponentManager) Stop() {
	// Make sure this only happens once.
	if atomic.AddInt32(&a.shutdown, 1) != 1 {
		log.Infof("Cryptixd is already in the process of shutting down")
		return
	}

	log.Warnf("Cryptixd shutting down")
...
func NewComponentManager(cfg *config.Config, db infrastructuredatabase.Database, interrupt chan<- struct{}) (
	*ComponentManager, error) {
	...
	consensusConfig := consensus.Config{
		...
		StartupRepairPlanPath:           cfg.StartupRepairPlan,
	}
