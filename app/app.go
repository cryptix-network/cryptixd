...
import (
	...
	"github.com/cryptix-network/cryptixd/infrastructure/os/execenv"
	"github.com/cryptix-network/cryptixd/infrastructure/os/limits"
	"github.com/cryptix-network/cryptixd/infrastructure/os/signal"
	"github.com/cryptix-network/cryptixd/infrastructure/os/winservice"
...
var serviceDescription = &winservice.ServiceDescription{
	Name:        "cryptixdsvc",
	DisplayName: "Cryptixd Service",
	Description: "Downloads and stays synchronized with the Cryptix blockDAG and " +
		"provides DAG services to applications.",
}

type cryptixdApp struct {
	cfg *config.Config
}

// StartApp starts the cryptixd app, and blocks until it finishes running
func StartApp() error {
...
func (app *cryptixdApp) main(startedChan chan<- struct{}) error {
	...
	if err != nil {
		log.Errorf("Unable to start cryptixd: %+v", err)
		return err
	}

	defer func() {
		log.Infof("Gracefully shutting down cryptixd...")
		...
		log.Infof("Cryptixd shutdown complete")
	}()
