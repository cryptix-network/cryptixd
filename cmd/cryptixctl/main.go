...
import (
	...
	"github.com/cryptix-network/cryptixd/infrastructure/network/rpcclient/grpcclient"
)
...
func main() {
	...
	if !cfg.AllowConnectionToDifferentVersions {
		cryptixdMessage, err := client.Post(&protowire.CryptixdMessage{Payload: &protowire.CryptixdMessage_GetInfoRequest{GetInfoRequest: &protowire.GetInfoRequestMessage{}}})
		if err != nil {
			printErrorAndExit(fmt.Sprintf("Cannot post GetInfo message: %s", err))
		}

		localVersion := version.Version()
		remoteVersion := cryptixdMessage.GetGetInfoResponse().ServerVersion
...
func prettifyResponse(response string) string {
	cryptixdMessage := &protowire.CryptixdMessage{}
	err := protojson.Unmarshal([]byte(response), cryptixdMessage)
	if err != nil {
	...
	return marshalOptions.Format(cryptixdMessage)
}
