...
// 2. A wrapper that implements isCryptixdMessage_Payload, having a single field pointing to the concrete command
//    bearing the name of the command with `CryptixdMessage_` prefix and `Request` suffix
...
// generateCryptixdMessage generates a wrapped CryptixdMessage with the given `commandValue`
func generateCryptixdMessage(commandValue reflect.Value, commandDesc *commandDescription) (*protowire.CryptixdMessage, error) {
	commandWrapper := reflect.New(commandDesc.typeof)
	unwrapCommandValue(commandWrapper).Set(commandValue)

	cryptixdMessage := reflect.New(reflect.TypeOf(protowire.CryptixdMessage{}))
	cryptixdMessage.Elem().FieldByName("Payload").Set(commandWrapper)
	return cryptixdMessage.Interface().(*protowire.CryptixdMessage), nil
}
