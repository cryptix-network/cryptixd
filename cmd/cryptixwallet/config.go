...
type sendConfig struct {
	...
	Password                 string   `long:"password" short:"p" description:"Wallet password"`
	DaemonAddress            string   `long:"daemonaddress" short:"d" description:"Wallet daemon server to connect to"`
	ToAddress                string   `long:"to-address" short:"t" description:"The public address to send Cryptix to" required:"true"`
	FromAddresses            []string `long:"from-address" short:"a" description:"Specific public address to send Cryptix from. Repeat multiple times (adding -a before each) to accept several addresses" required:"false"`
	SendAmount               string   `long:"send-amount" short:"v" description:"An amount to send in Cryptix (e.g. 1234.12345678)"`
	IsSendAll                bool     `long:"send-all" description:"Send all the Cryptix in the wallet (mutually exclusive with --send-amount). If --from-address was used, will send all only from the specified addresses."`
	...
	MaxFee                   uint64   `long:"max-fee" short:"x" description:"Maximum fee in Sompi (not Sompi/gram) to use for the transaction. The wallet will take the minimum between the fee estimate from the connected node and this value. If no other fee policy is specified, it will set the max fee to 1 CYTX"`
...
type createUnsignedTransactionConfig struct {
	DaemonAddress            string   `long:"daemonaddress" short:"d" description:"Wallet daemon server to connect to"`
	ToAddress                string   `long:"to-address" short:"t" description:"The public address to send Cryptix to" required:"true"`
	FromAddresses            []string `long:"from-address" short:"a" description:"Specific public address to send Cryptix from. Use multiple times to accept several addresses" required:"false"`
	SendAmount               string   `long:"send-amount" short:"v" description:"An amount to send in Cryptix (e.g. 1234.12345678)"`
	IsSendAll                bool     `long:"send-all" description:"Send all the Cryptix in the wallet (mutually exclusive with --send-amount)"`
	UseExistingChangeAddress bool     `long:"use-existing-change-address" short:"u" description:"Will use an existing change address (in case no change address was ever used, it will use a new one)"`
	MaxFeeRate               float64  `long:"max-fee-rate" short:"m" description:"Maximum fee rate in Sompi/gram to use for the transaction. The wallet will take the minimum between the fee rate estimate from the connected node and this value."`
	FeeRate                  float64  `long:"fee-rate" short:"r" description:"Fee rate in Sompi/gram to use for the transaction. This option will override any fee estimate from the connected node."`
	MaxFee                   uint64   `long:"max-fee" short:"x" description:"Maximum fee in Sompi (not Sompi/gram) to use for the transaction. The wallet will take the minimum between the fee estimate from the connected node and this value. If no other fee policy is specified, it will set the max fee to 1 CYTX"`
...
func parseCommandLine() (subCommand string, config interface{}) {
	...
	parser.AddCommand(balanceSubCmd, "Shows the balance of a public address",
		"Shows the balance for a public address in Cryptix", balanceConf)

	sendConf := &sendConfig{DaemonAddress: defaultListen}
	parser.AddCommand(sendSubCmd, "Sends a Cryptix transaction to a public address",
		"Sends a Cryptix transaction to a public address", sendConf)
	...
	parser.AddCommand(createUnsignedTransactionSubCmd, "Create an unsigned Cryptix transaction",
		"Create an unsigned Cryptix transaction", createUnsignedTransactionConf)
