...
func TestMultisig(t *testing.T) {
	testutils.ForAllNets(t, true, func(t *testing.T, consensusConfig *consensus.Config) {
		...
		forSchnorrAndECDSA(t, func(t *testing.T, ecdsa bool) {
			...
			for i := 0; i < numKeys; i++ {
				var err error
				mnemonics[i], err = libcryptixwallet.CreateMnemonic()
				if err != nil {
					t.Fatalf("CreateMnemonic: %+v", err)
				}

				publicKeys[i], err = libcryptixwallet.MasterPublicKeyFromMnemonic(&consensusConfig.Params, mnemonics[i], true)
				if err != nil {
			...
			path := "m/1/2/3"
			address, err := libcryptixwallet.Address(params, publicKeys, minimumSignatures, path, ecdsa)
			if err != nil {
			...
			selectedUTXOs := []*libcryptixwallet.UTXO{
				{
			...
			unsignedTransaction, err := libcryptixwallet.CreateUnsignedTransaction(publicKeys, minimumSignatures,
				[]*libcryptixwallet.Payment{{
					Address: address,
					Amount:  10,
				}}, selectedUTXOs)
			if err != nil {
			...
			isFullySigned, err := libcryptixwallet.IsTransactionFullySigned(unsignedTransaction)
			if err != nil {
			...
			_, err = libcryptixwallet.ExtractTransaction(unsignedTransaction, ecdsa)
			...
			signedTxStep1, err := libcryptixwallet.Sign(params, mnemonics[:1], unsignedTransaction, ecdsa)
			if err != nil {
				t.Fatalf("Sign: %+v", err)
			}

			isFullySigned, err = libcryptixwallet.IsTransactionFullySigned(signedTxStep1)
			if err != nil {
				t.Fatalf("IsTransactionFullySigned: %+v", err)
			}
			...
			signedTxStep2, err := libcryptixwallet.Sign(params, mnemonics[1:2], signedTxStep1, ecdsa)
			if err != nil {
				t.Fatalf("Sign: %+v", err)
			}

			extractedSignedTxStep2, err := libcryptixwallet.ExtractTransaction(signedTxStep2, ecdsa)
			if err != nil {
				t.Fatalf("ExtractTransaction: %+v", err)
			}

			signedTxOneStep, err := libcryptixwallet.Sign(params, mnemonics[:2], unsignedTransaction, ecdsa)
			if err != nil {
				t.Fatalf("Sign: %+v", err)
			}

			extractedSignedTxOneStep, err := libcryptixwallet.ExtractTransaction(signedTxOneStep, ecdsa)
			if err != nil {
				t.Fatalf("ExtractTransaction: %+v", err)
			}
	...
	testutils.ForAllNets(t, true, func(t *testing.T, consensusConfig *consensus.Config) {
		...
		forSchnorrAndECDSA(t, func(t *testing.T, ecdsa bool) {
			...
			for i := 0; i < numKeys; i++ {
				...
				publicKeys[i], err = libcryptixwallet.MasterPublicKeyFromMnemonic(&consensusConfig.Params, mnemonics[i], false)
				if err != nil {
					t.Fatalf("MasterPublicKeyFromMnemonic: %+v", err)
				}
			}
			...
			signedTx, err := libcryptixwallet.Sign(params, mnemonics, unsignedTransaction, ecdsa)
			if err != nil {
				t.Fatalf("Sign: %+v", err)
			}

			tx, err := libcryptixwallet.ExtractTransaction(signedTx, ecdsa)
			if err != nil {
				t.Fatalf("ExtractTransaction: %+v", err)
			}
...
func TestMaxSompi(t *testing.T) {
	testutils.ForAllNets(t, true, func(t *testing.T, consensusConfig *consensus.Config) {
		...
		cfg.PreDeflationaryPhaseBaseSubsidy = 20e6 * constants.SompiPerCryptix
		...
		for i := 0; i < numKeys; i++ {
			...
			publicKeys[i], err = libcryptixwallet.MasterPublicKeyFromMnemonic(&cfg.Params, mnemonics[i], false)
			if err != nil {
				t.Fatalf("MasterPublicKeyFromMnemonic: %+v", err)
			}
		}
		...
		const minimumSignatures = 1
		path := "m/1/2/3"
		address, err := libcryptixwallet.Address(params, publicKeys, minimumSignatures, path, false)
		if err != nil {
			t.Fatalf("Address: %+v", err)
		}
		...
		selectedUTXOsForTxWithLargeInputAmount := []*libcryptixwallet.UTXO{
			{
				Outpoint: &externalapi.DomainOutpoint{
		...
		unsignedTxWithLargeInputAmount, err := libcryptixwallet.CreateUnsignedTransaction(publicKeys, minimumSignatures,
			[]*libcryptixwallet.Payment{{
				Address: address,
				Amount:  10,
		...
		signedTxWithLargeInputAmount, err := libcryptixwallet.Sign(params, mnemonics, unsignedTxWithLargeInputAmount, false)
		if err != nil {
			t.Fatalf("Sign: %+v", err)
		}

		txWithLargeInputAmount, err := libcryptixwallet.ExtractTransaction(signedTxWithLargeInputAmount, false)
		if err != nil {
			t.Fatalf("ExtractTransaction: %+v", err)
		}
		...
		selectedUTXOsForTxWithLargeInputAndOutputAmount := []*libcryptixwallet.UTXO{
			{
				Outpoint: &externalapi.DomainOutpoint{
		...
		unsignedTxWithLargeInputAndOutputAmount, err := libcryptixwallet.CreateUnsignedTransaction(publicKeys, minimumSignatures,
			[]*libcryptixwallet.Payment{{
				Address: address,
				Amount:  22e6 * constants.SompiPerCryptix,
			}}, selectedUTXOsForTxWithLargeInputAndOutputAmount)
		if err != nil {
			t.Fatalf("CreateUnsignedTransactions: %+v", err)
		}

		signedTxWithLargeInputAndOutputAmount, err := libcryptixwallet.Sign(params, mnemonics, unsignedTxWithLargeInputAndOutputAmount, false)
		if err != nil {
			t.Fatalf("Sign: %+v", err)
		}

		txWithLargeInputAndOutputAmount, err := libcryptixwallet.ExtractTransaction(signedTxWithLargeInputAndOutputAmount, false)
		if err != nil {
			t.Fatalf("ExtractTransaction: %+v", err)
		}
