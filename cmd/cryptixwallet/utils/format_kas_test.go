...
// Takes in a string representation of the Cytx value to convert to Sompi
func TestCytxToSompi(t *testing.T) {
	...
	for _, currentTestVector := range validCases {
		convertedAmount, err := CytxToSompi(currentTestVector.originalAmount)
	...
	for _, currentTestVector := range invalidCases {
		_, err := CytxToSompi(currentTestVector)
...
func TestValidateAmountFormat(t *testing.T) {
	...
	for _, testCase := range validCases {
		err := validateCYTXAmountFormat(testCase)
