...
// Takes in a string representation of the Cpay value to convert to Sompi
func TestCpayToSompi(t *testing.T) {
	...
	for _, currentTestVector := range validCases {
		convertedAmount, err := CpayToSompi(currentTestVector.originalAmount)
	...
	for _, currentTestVector := range invalidCases {
		_, err := CpayToSompi(currentTestVector)
...
func TestValidateAmountFormat(t *testing.T) {
	...
	for _, testCase := range validCases {
		err := validateCPAYAmountFormat(testCase)
