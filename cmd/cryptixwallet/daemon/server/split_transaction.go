...
func (s *server) mergeTransaction(
	...
	return libcryptixwallet.CreateUnsignedTransaction(s.keysFile.ExtendedPublicKeys,
...
func createTransactionWithJunkFieldsForMassCalculation(transaction *serialization.PartiallySignedTransaction, ecdsa bool, minimumSignatures uint32, txMassCalculator *txmass.Calculator) (*externalapi.DomainTransaction, error) {
	...
	return libcryptixwallet.ExtractTransactionDeserialized(transaction, ecdsa)
}
...
func (s *server) moreUTXOsForMergeTransaction(alreadySelectedUTXOs []*libcryptixwallet.UTXO, requiredAmount uint64, feeRate float64) (
