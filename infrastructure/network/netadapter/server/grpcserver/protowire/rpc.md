...
Clients are expected to build RequestMessages and wrap them in CryptixdMessage. (see messages.proto)

Having received a RequestMessage, (wrapped in a CryptixdMessage) the RPC server will respond with a
ResponseMessage (likewise wrapped in a CryptixdMessage) respective to the original RequestMessage.
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| payAddress | [string](#string) |  | Which cryptix address should the coinbase block reward transaction pay into |
...
| ----- | ---- | ----- | ----------- |
| block | [RpcBlock](#protowire.RpcBlock) |  |  |
| isSynced | [bool](#bool) |  | Whether cryptixd thinks that it&#39;s synced. Callers are discouraged (but not forbidden) from solving blocks when cryptixd is not synced. That is because when cryptixd isn&#39;t in sync with the rest of the network there&#39;s a high chance the block will never be accepted, thus the solving effort would have been wasted. |
| error | [RPCError](#protowire.RPCError) |  |  |
...
| isOutbound | [bool](#bool) |  | Whether this cryptixd initiated the connection |
| timeOffset | [int64](#int64) |  |  |
| userAgent | [string](#string) |  |  |
| advertisedProtocolVersion | [uint32](#uint32) |  | The protocol version that this peer claims to support |
| timeConnected | [int64](#int64) |  | The timestamp of when this peer connected to this cryptixd |
